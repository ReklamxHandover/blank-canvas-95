...
import { CheckCircle2, Upload, Paperclip, Download } from 'lucide-react';
...
export default function ProducerView() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const grouped = {};
      data.forEach(a => { (grouped[a.order_id] = grouped[a.order_id] || []).push(a); });
  ...
  const handleMarkDone = (orderId) => {
    updatePipelineStage(orderId, 'produktion', 'done', '');
    updatePipelineStage(orderId, 'faktura', 'active', '');
  };

  const handleChecklistChange = (order, productId, nextChecklist) => {
    const nextProducts = (order.products || []).map(p =>
      p.id === productId ? { ...p, production_checklist: nextChecklist } : p
    );
    updateOrder(order.id, { products: nextProducts });
  };
            ...
            <div key={order.id} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
                ...
                <div style={{
                  ...
                  {t(`status_${order.pipeline.produktion.status}`)}
                </div>
              </div>
                ...
                {order.products.map(p => (
                  <div key={p.id} style={{ padding: '12px 16px', background: '#faf9f7', borderRadius: 10, border: '1px solid #f0ede8' }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: '#1a1a2e', marginBottom: 4 }}>
                      {p.name} <span style={{ color: '#b0b8c4', fontWeight: 400, fontSize: 13 }}>× {p.quantity} {t('pcs')}</span>
                    </div>
                    <div style={{ fontSize: 12, color: '#7a8394', lineHeight: 1.6 }}>{p.specs}</div>
                    <ProductionChecklist
                      value={p.production_checklist}
                      onChange={(next) => handleChecklistChange(order, p.id, next)}
                    />
                  </div>
                ))}
              </div>
              ...
              {order.pipeline.produktion.notes && (
                <div style={{ fontSize: 13, color: '#7a8394', fontStyle: 'italic', marginBottom: 16 }}>
                  {order.pipeline.produktion.notes}
                </div>
              )}

              {/* Attachments (designer files etc.) */}
              {atts.length > 0 && (
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <button
                  ...
                  style={{
                    padding: '11px 18px', background: '#ffffff', border: '1px solid #e8e4df',
                    borderRadius: 10, fontSize: 13, fontWeight: 600, color: '#1a1a2e',
                    cursor: uploadingFor === order.id ? 'wait' : 'pointer',
                    display: 'inline-flex', alignItems: 'center', gap: 7,
                  }}
                >
                  <Upload size={14} strokeWidth={2} />
                  {uploadingFor === order.id ? 'Laddar upp…' : t('uploadProductionFile')}
                </button>
                <button
